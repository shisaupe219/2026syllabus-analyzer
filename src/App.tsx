...
export default function App() {
  ...
  const guideSteps = [
    ...
    {
      title: "最后：导出报告",
      content: "确认无误后，您可以一键导出精美的 Word 或 HTML 格式的分析报告。",
  ...
  const exportHtml = async () => {
    ...
    setUploadError(null);
    setErrorType('export');
    setLoadingMessage('正在准备 HTML 报告内容...');
    ...
    try {
      const header = `
        ...
        <head>
          ...
          <title>${courseInfo.courseName || '课程'}达成度分析报告</title>
          <style>
            body { 
            ...
            p.caption, .fig-caption, .chart-caption { 
              text-indent: 0; 
              text-align: center; 
              font-size: 9pt; 
              margin: 4px 0 12px;
            ...
            /* Reset indent for table content */
        ...
        <body contenteditable="true">
          <div class="no-print" style="text-align: right; margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 10px;" contenteditable="false">
            <span style="float: left; color: #666; font-size: 10pt; line-height: 32px;">提示：报告内容可直接点击修改，完成后可点击右侧按钮保存为 PDF。</span>
            <button onclick="window.print()" style="padding: 8px 16px; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold;">打印报告 / 保存为 PDF</button>
          </div>
      `;
      ...
      // Wrap tables in a scrollable container for better responsiveness
      const tables = reportClone.querySelectorAll('table');
      tables.forEach(table => {
        ...
        if (originalContainer) {
          ...
          try {
            ...
            if (svgElement) {
              ...
              const bbox = svgElement.getBBox ? svgElement.getBBox() : { width: 800, height: 400 };
              const rect = svgElement.getBoundingClientRect();
              const originalWidth = rect.width || bbox.width || 800;
              const originalHeight = rect.height || bbox.height || 400;
              ...
              // Set explicit width and height for high res (2x original)
              const exportWidth = originalWidth * 2;
              const exportHeight = originalHeight * 2;
              clonedSvg.setAttribute('width', exportWidth.toString());
              clonedSvg.setAttribute('height', exportHeight.toString());
              
              // Remove root level clipping
              clonedSvg.style.overflow = 'visible';
              ...
              if (response.ok) {
                ...
                let captionText = '';
                if (id === 'chart-histogram') captionText = '图1 综合成绩分布直方图';
                else if (id === 'chart-pie') captionText = '图2 各分数段占比饼图';
                else if (id === 'chart-scatter') captionText = '图3 平时成绩 vs 考试成绩散点图';
                else if (id.startsWith('chart-objective-')) {
                  const index = parseInt(id.replace('chart-objective-', '')) + 1;
                  captionText = `图${index + 3} 课程目标${index}学生达成情况分布图`;
                }
            ...
            console.warn(`Error using backend for chart ${id} in HTML export, falling back to html2canvas:`, err);
            try {
              const canvas = await html2canvas(originalContainer, { 
                scale: 3,
                useCORS: true,
              ...
              console.error(`Fallback failed for chart ${id} in HTML export:`, fallbackErr);
            }
          }
        }
      }
      ...
      // Final cleanup of img tags and handle highlighted numbers [26]{.mark}
      const finalReportHtml = content.replace(/\[([^\]]+)\]\{\.mark\}/g, '<span class="mark">$1</span>')
                                   .replace(/<img([^>]*)width\s*=\s*["'][^"']*["']([^>]*)>/gi, '<img$1$2>')
                                   .replace(/<img([^>]*)height\s*=\s*["'][^"']*["']([^>]*)>/gi, '<img$1$2>');

      const blob = new Blob([header + finalReportHtml + footer], { type: 'text/html' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `${courseInfo.courseName || '课程'}_达成度分析报告.html`;
      link.click();
      URL.revokeObjectURL(url);
      
    } catch (error) {
      console.error('HTML Export Error:', error);
      setUploadError('生成 HTML 报告失败。');
      setErrorType('export');
    } finally {
      setIsProcessing(false);
      setIsExporting(false);
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="flex gap-3" id="header-actions">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-200"
            >
